    ...
    <NavLink
      ...
      className={({ isActive }) =>
        cn(
          ...
          isActive
            ? 'bg-[rgba(189,167,136,0.12)] text-[#f4f1e6]'
            : 'text-[rgba(244,241,230,0.5)] hover:text-[rgba(244,241,230,0.85)] hover:bg-[rgba(244,241,230,0.05)]'
        )
      }
    >
      {({ isActive }) => (
        <>
          ...
          <Icon
            ...
            className={isActive ? 'text-[#bda788]' : ''}
          />
...
export default function Sidebar() {
  ...
  const sidebarContent = (
    <div className="flex flex-col h-full" style={{ background: '#2d2d2d' }}>
      ...
      <div className="px-5 pt-7 pb-6 flex flex-col items-center gap-3">
        ...
        <div className="text-center">
          <p className="font-display text-[#f4f1e6] text-sm tracking-[0.12em] uppercase leading-none">
            Mentoria
          </p>
          <p
        ...
        <div
          className="w-10 h-px"
          style={{ background: 'linear-gradient(90deg, transparent, #bda788, transparent)' }}
        />
      </div>
      ...
      <div
        ...
        style={{ background: 'rgba(244,241,230,0.05)', border: '1px solid rgba(244,241,230,0.07)' }}
      >
        <div className="flex items-center gap-3">
          <Avatar src={user?.avatar_url} name={user?.name} size="sm" gold />
          <div className="min-w-0 flex-1">
            <p className="text-sm font-body font-medium text-[#f4f1e6] truncate leading-none mb-0.5">
              {user?.name}
            </p>
            {user?.instagram && (
              <p className="text-[11px] font-body truncate" style={{ color: 'rgba(189,167,136,0.6)' }}>
                @{user.instagram}
              </p>
            )}
          </div>
          <button
            onClick={logout}
            className="p-1.5 rounded-lg transition-colors hover:bg-[rgba(244,241,230,0.08)]"
            title="Sair"
          >
            <LogOut size={14} style={{ color: 'rgba(244,241,230,0.35)' }} />
          </button>
        </div>
      </div>
    </div>
  ...
  return (
    <>
      ...
      <aside className="hidden lg:flex w-60 flex-col h-screen sticky top-0 flex-shrink-0"
        style={{ background: '#2d2d2d', borderRight: '1px solid rgba(244,241,230,0.07)' }}>
        {sidebarContent}
      </aside>
      ...
      <button
        ...
        style={{ background: '#2d2d2d', border: '1px solid rgba(244,241,230,0.12)' }}
        onClick={() => setMobileOpen(true)}
      >
        <Menu size={20} style={{ color: '#f4f1e6' }} />
      </button>
      ...
      >
        <button
          ...
          style={{ color: 'rgba(244,241,230,0.4)' }}
          onClick={() => setMobileOpen(false)}
        >
          <X size={18} />
        </button>
