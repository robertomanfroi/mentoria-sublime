...
function listUsers() {
  const totalChecklist = db.prepare('SELECT COUNT(*) as cnt FROM checklist_items WHERE active = 1').get().cnt;
  const currentMonth = new Date().toISOString().slice(0, 7); // YYYY-MM

  const users = db.prepare(`
    SELECT u.id, u.name, u.email, u.instagram_handle, u.profile_photo, u.role, u.created_at,
      (SELECT COUNT(*) FROM checklist_progress cp WHERE cp.user_id = u.id AND cp.completed = 1) as checklist_completed
    FROM users u WHERE u.role = 'mentorada' ORDER BY u.name ASC
  `).all();

  // Busca dados mensais do mês atual para cada mentorada
  const monthlyMap = {};
  const monthlyRows = db.prepare('SELECT * FROM monthly_data WHERE month = ?').all(currentMonth);
  monthlyRows.forEach(r => { monthlyMap[r.user_id] = r; });

  // Busca último snapshot de ranking
  const snapshotMap = {};
  ...
  return users.map(u => {
    const md = monthlyMap[u.user_id] || monthlyMap[u.id];
    const snap = snapshotMap[u.id];
    const checklistPct = totalChecklist > 0
      ? Math.round((u.checklist_completed / totalChecklist) * 100)
      : 0;
    ...
    return {
      ...
      checklist_completed: u.checklist_completed,
...
function updateUser(id, { name, email, instagram_handle, role }) {
  ...
  return db.prepare('SELECT id, name, email, instagram_handle, role, created_at FROM users WHERE id = ?').get(id);
}
...
function listChecklistItems() {
  return db.prepare('SELECT * FROM checklist_items ORDER BY sort_order ASC').all();
}

function addChecklistItem({ stage, description, sort_order }) {
...
function updateChecklistItem(id, { stage, description, sort_order, active }) {
  const existing = db.prepare('SELECT id FROM checklist_items WHERE id = ?').get(id);
  if (!existing) {
...
function listPendingValidations() {
  return db.prepare(`
...
function setValidation(id, approved) {
  const existing = db.prepare('SELECT id FROM monthly_data WHERE id = ?').get(id);
  if (!existing) {
    const err = new Error('Registro não encontrado.'); err.status = 404; throw err;
  }
  const validated = approved ? 1 : 0;
  db.prepare('UPDATE monthly_data SET validated_by_admin = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?').run(validated, id);
  return db.prepare('SELECT * FROM monthly_data WHERE id = ?').get(id);
}

// ─── PRIZES ───────────────────────────────────────────────────────────────────

function listAllPrizes() {
...
function calculateAndSaveRanking(month) {
  if (!/^\d{4}-\d{2}$/.test(month)) {
    const err = new Error('Formato de mês inválido. Use YYYY-MM.'); err.status = 400; throw err;
  }

  const allMonthlyData = db.prepare('SELECT * FROM monthly_data WHERE month = ?').all(month);
  ...
  // Remove snapshots antigos do mês
  db.prepare('DELETE FROM ranking_snapshots WHERE month = ?').run(month);
  ...
  const insertMany = db.transaction((items) => {
    items.forEach((s, i) => {
      insert.run(s.user_id, month, s.checklist_score, s.revenue_score, s.followers_score, s.total_score, i + 1);
    });
  });
...
function exportCSV() {
  ...
  const rows = users.map(u => {
    ...
    const latestSnapshot = db.prepare(`
      SELECT * FROM ranking_snapshots WHERE user_id = ? ORDER BY month DESC LIMIT 1
    `).get(u.id);
  ...
  const headers = [
    'Nome', 'E-mail', 'Instagram', 'Checklist Concluídos', 'Total Itens',
    '% Checklist', 'Último Mês', 'Score Checklist', 'Score Seguidores',
    'Score Total', 'Posição'
  ];
...
// Configurações persistidas no banco (tabela key-value)
...
function updateSettings(newSettings) {
  ...
  try {
    ...
    // Tabela pode não existir em ambientes legados — usa memória como fallback
  }
  return merged;
}
